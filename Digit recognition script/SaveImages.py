...
def saveImages():
    ...
    image = ~np.array(list(file_content[16:800])).reshape(28,28).astype(np.uint8) ### Teshape the list into a 28*28 grid image.

    myInt = int.from_bytes(test_lbl[9:10], byteorder="big") ### Tead the next bit into the myInt variable.
    print("The next number is " + str(myInt)) ### Tonvert the int to a string and output to the screen.
